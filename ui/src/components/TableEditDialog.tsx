...
const TableEditDialog: Component<TableEditDialogProps> = (props) => {
  const [tableName, setTableName] = createSignal(props.table?.title || '')
  const [tableComment, setTableComment] = createSignal(props.table?.comment || '')
  const [columns, setColumns] = createSignal<Column[]>(props.table ? [...props.table.columns] : [])
  const [primaryKeys, setPrimaryKeys] = createSignal<string[]>(props.table?.primaryKey || [])
  const [uniqueConstraints, setUniqueConstraints] = createSignal<Index[]>(props.table?.uniqueConstraints || [])
  const [indexes, setIndexes] = createSignal<Index[]>(props.table?.indexes || [])
  ...
  createEffect(() => {
    if (props.table && props.isOpen) {
      setTableName(props.table.title)
      setTableComment(props.table.comment || '')
      setColumns([...props.table.columns])
      setPrimaryKeys(props.table.primaryKey || [])
      setUniqueConstraints(props.table.uniqueConstraints || [])
      setIndexes(props.table.indexes || [])
    }
  })
